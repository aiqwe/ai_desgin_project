...
def negative_sampling(example, positive):
...
def main():
    parser = HfArgumentParser((ModelArguments, DataArguments, SentenceTransformerTrainingArguments))
    ...
    if (check_dataset("train")) and (check_dataset("test")):

        if check_dataset("train"):
            train_dataset = load_dataset("json", data_files = train_dataset_path, split = "train")

        if check_dataset("test"):
            test_dataset = load_dataset("json", data_files = test_dataset_path, split = "train")

    else:
        dataset = load_dataset("json", data_files = data_args.dataset, split = "train")
        positive = dataset['text']
        negative = deepcopy(positive)
        
        dataset = dataset.map(partial(negative_sampling, positive=positive), remove_columns = ['title', 'text', 'url'])
    ...
    model = SentenceTransformer(model_args.model_path, model_kwargs=model_kwargs, device = model_args.device)

    # Train
    wandb.login(key=_getenv("WANDB_API_KEY"))
    loss = TripletLoss(model)
    training_args.hub_token =(_getenv("HF_WRITE_TOKEN"))
